...
class MySequentialFlow(SequentialFlow):
    ...
    def forward(
        ...
        if not isinstance(x, torch.Tensor):
            x = torch.as_tensor(x, dtype=torch.float32, device=self.device if hasattr(self, "device") else None)
        if not isinstance(y, torch.Tensor):
            y = torch.as_tensor(y, dtype=torch.float32, device=self.device if hasattr(self, "device") else None)
            
        batch_shape = x.shape[:-1]
        sample_shape = y.shape[:-(len(batch_shape) + 1)]

        def _normalize_sample_shape(sample_shape):
            if isinstance(sample_shape, torch.Size):
                if len(sample_shape) == 0:   # torch.Size([])
                    return ()
                return tuple(sample_shape)
            elif isinstance(sample_shape, torch.Tensor):
                # Tensor scalaire
                return (int(sample_shape.item()),)
            elif isinstance(sample_shape, int):
                return (sample_shape,)
            elif isinstance(sample_shape, tuple):
                return sample_shape
            else:
                raise ValueError(f"Unsupported sample_shape type: {type(sample_shape)}")
            
        if not isinstance(x, torch.Tensor):
            x = torch.tensor(x, dtype=torch.float32, device=self.device if hasattr(self, "device") else None)

        sample_shape = _normalize_sample_shape(sample_shape)

        x_repeat = x.view((1,) * len(sample_shape) + tuple(x.shape)) \
                .expand(sample_shape + tuple(x.shape))
