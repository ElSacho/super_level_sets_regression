...
class RobustLambdaHeadMVCS(nn.Module):
    """
    Predicts mu and the Cholesky factor L.
    The matrix Lambda is defined as Lambda = L @ L.T
    """
    def __init__(self, input_dim, y_dim, init_scale=1.0, init_p=2.0):
        super().__init__()
        self.y_dim = y_dim
        ...
        # Initialization
        with torch.no_grad():
            # If Lambda ~ Identity, then L ~ Identity
            diag_mask = (self.tril_indices[0] == self.tril_indices[1])
            inv_softplus_diag = math.log(math.exp(init_scale) - 1)
            self.fc_chol.bias.zero_()
            self.fc_chol.bias[diag_mask] = inv_softplus_diag

    @property
    def p(self):
        """Returns the constrained p value (p > 1)"""
        return self.p_raw
        # return F.softplus(self.p_raw) + 1.0
    ...
    def forward(self, x):
        ...
        # Enforce positive diagonal for L
        diag_idx = torch.arange(self.y_dim, device=x.device)
        L[:, diag_idx, diag_idx] = F.softplus(L[:, diag_idx, diag_idx]) + 1e-6
        
        # We return L here to compute determinants efficiently in the loss
        # Lambda = L @ L.T is computed where needed
        return (mu, L)

    def _log_lambda_constant(self, k, p):
        """
        Calculates log lambda(B ||.||p (1))
        """
    ...
    def loss(self, params, y_target, alpha):
        """
        Loss = log(mean(1/det(Lambda))) + k * log(sigma_r) + const
        """
        mu, L = params
        B = y_target.shape[0]
        r = int(alpha * B)
        ...
        # --- 1. Compute Log Determinant of Lambda ---
        # Lambda = L @ L.T
        # det(Lambda) = det(L) * det(L^T) = det(L)^2
        # log det(Lambda) = 2 * log det(L) = 2 * sum(log(diag(L)))
        diag_L = torch.diagonal(L, dim1=-2, dim2=-1)
        log_det_lambda = 2.0 * torch.sum(torch.log(diag_L), dim=1) # Shape: (B,)
        
        # --- Term 1: log( (1/n) * sum( 1 / det(Lambda_i) ) ) ---
        # We use LogSumExp for stability:
        # log(sum(1/D)) = log(sum(exp(-log D)))
        # Here log D = log_det_lambda
        # So we compute LogSumExp( -log_det_lambda ) - log(n)
        term1 = torch.logsumexp(-log_det_lambda, dim=0) - math.log(B)

        # --- 2. Compute Whitened Norms ---
        # Formula: || Lambda * (y - mu) ||_p
        # Lambda = L @ L.T
        Lambda = torch.matmul(L, L.transpose(1, 2))
        
        residual = (y_target - mu).unsqueeze(-1) # (B, D, 1)
        
        # z = Lambda * residual
        z = torch.matmul(Lambda, residual).squeeze(-1) # (B, D)
        
        # Calculate p-norm of z
        whitened_norm = (torch.abs(z).pow(current_p).sum(dim=1)).pow(1.0 / current_p)

        # Sort to find rank statistic sigma_r
        _, sorted_idx = torch.sort(whitened_norm, descending=True)
...
class MatrixPredictorMVCS:
    ...
    def __call__(self, x):
        ...
        with torch.no_grad():
            feats = self.backbone(x)
            mu, L = self.head(feats)
            # Reconstruct Lambda = L L^T
            Lambda = torch.matmul(L, L.transpose(1, 2))
            return Lambda
...
class MVCSRobustTabularModel(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim=128, num_layers=3, dropout=0.1):
        ...
        self.head = RobustLambdaHeadMVCS(hidden_dim, output_dim, init_p=2.0)
